...
with col3:
    ...
    profit_margin = (filtered_df['Profit'].sum() / total_sales * 100) if total_sales > 0 else 0
...
with col6:
    ...
    st.metric("Avg Quantity", f"{avg_quantity:.2f}")
...
try:
    ...
    st.error(f"Error displaying profit charts: {e}")
...
try:
    ...
    st.error(f"Error displaying segment charts: {e}")
...
try:
    ...
    st.error(f"Error displaying yearly charts: {e}")
...
try:
    ...
    st.error(f"Error displaying sub-category chart: {e}")
...
try:
    ...
    st.error(f"Error displaying orders chart: {e}")
